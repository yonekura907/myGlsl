...
/** ===========================================================================
 * 行列を利用すると、3D シーンを描画するための「頂点の座標変換」が行いやすくなる
 * ため、一般的にも 3D プログラミングにおいて行列はとても重要な役割を果たします。
 * このサンプルでは、行列を利用して 3D シーンをレンダリングするための、最低限の
 * 準備を行っています。
 * 行列を処理するための仕組みや算術関数（行列などを処理する関数）は、JavaScript
 * には仕様上存在しないため、なにかしら独自に行列に関する処理を行うライブラリな
 * どを準備しなければなりません。
 * ここでは、自作の minMatrix.js を利用しています。
 * ========================================================================= */
...
class WebGLFrame {
    ...
    load(){
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                // attribute 変数関係
                this.attLocation = [
                    gl.getAttribLocation(this.program, 'position'), // 頂点の座標
                    gl.getAttribLocation(this.program, 'color'),    // 頂点の色
                ];
                this.attStride = [
                    3,
                    4,
                ...
                // uniform 変数関係
                this.uniLocation = [
                    gl.getUniformLocation(this.program, 'mvpMatrix'), // MVP 座標変換行列
                    gl.getUniformLocation(this.program, 'time'),      // 時間の経過
                ];
                this.uniType = [
                    'uniformMatrix4fv', // 行列は uniformMatrix + 数値 + fv のタイプを指定する @@@
                    'uniform1f',
                ];
    ...
    setup(){
        ...
        /**
         * シーンが 3D になるので、クリアする色だけでなく、奥行きを判定するための
         * 深度バッファのクリア時の初期値も指定してやる必要があります。
         * 深度バッファはその名のとおり、奥行き（深度）の情報を格納しておくための
         * バッファで、頂点が描かれるたびに逐一更新されていきます。ただしこのとき、
         * 新しく描画しようとした頂点よりも手前に、他の頂点やポリゴンが存在するか
         * どうかを深度バッファの内容を確認してチェックし、もし手前になにかある場
         * 合は頂点の描画を行わない、といった処理が行われます。これにより、前後関
         * 係が正しく維持されます。
         * この仕組を「深度テスト」と呼びます。
         */
    ...
    render(){
        ...
        this.setUniform([
            this.mvpMatrix, // JavaScript（CPU 側）で計算した行列をシェーダに送る @@@
            this.nowTime,
