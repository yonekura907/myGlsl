...
/** ===========================================================================
 * VBO を生成する際にもともと頂点が存在している 3D 空間上の位置のことを、一般に
 * ローカル座標と呼びます。
 * これまでのサンプルでは、ローカル座標は XZ 平面に平たく敷き詰められたような状
 * 態になっていましたが、ここではそれを筒状に配置されるようにしています。
 * いったいどのような計算を行えば頂点のローカル座標を円筒形に配置できるのか、自
 * 分なりに考えながらコードを読み解いてみましょう。
 * ========================================================================= */
...
class WebGLFrame {
    ...
    setup(){
        ...
        const VERTEX_RADIUS = 0.5; // 円筒の半径
        this.position = [];        // 頂点座標
        this.color = [];           // 頂点色
        for(let i = 0; i < VERTEX_COUNT; ++i){
            // 変数 i を元にラジアンを求める @@@
            let ratio = (i / VERTEX_COUNT) * Math.PI * 2.0;
            // 求めたラジアンからサインとコサインを作る @@@
            let x = Math.sin(ratio) * VERTEX_RADIUS;
            let z = Math.cos(ratio) * VERTEX_RADIUS;
