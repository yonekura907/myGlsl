...
/** ===========================================================================
 * せっかくテクスチャを使った処理を行いましたので、複数のテクスチャを同時に利用
 * する方法についても確認しておきましょう。
 * ここでは、同時に２つのテクスチャをシェーダに送ったあと、シェーダ側では色と色
 * とを mix 関数で補間することで、２つの画像間を遷移するような処理を行っています。
 * ========================================================================= */

const MAT = new matIV(); // minMatrix.js に定義されている行列処理クラス
const QTN = new qtnIV(); // minMatrix.js に定義されているクォータニオン処理クラス

let mixRatio = 0.0; // テクスチャの合成割合い @@@

window.addEventListener('DOMContentLoaded', () => {
    // https://github.com/cocopon/tweakpane
    // スライダーコントロール作成には Tweakpane を使わせていただいてます。
    // cocopon さんありがとう！
...
class WebGLFrame {
    constructor(){
        ...
        this.texture = []; // 複数のテクスチャを使うので配列にしておく
    }
    /**
        ...
        return new Promise((resolve) => {
            ...
            .then((shaders) => {
                ...
                this.uniLocation = [
                    ...
                    gl.getUniformLocation(this.program, 'ratio'),        // 合成割合い @@@
                    gl.getUniformLocation(this.program, 'textureUnit0'), // テクスチャ @@@
                    gl.getUniformLocation(this.program, 'textureUnit1'), // テクスチャ @@@
                ];
                this.uniType = [
                    'uniformMatrix4fv',
                    'uniform1f',
